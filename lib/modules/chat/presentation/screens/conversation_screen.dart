...
class _ConversationScreenState extends ConsumerState<ConversationScreen> {
  ...
  void initState() {
    super.initState();
    debugPrint(
        '📱 ConversationScreen: initState with conversationId: ${widget.conversationId}');
  ...
  void didUpdateWidget(ConversationScreen oldWidget) {
    ...
    if (widget.conversationId != oldWidget.conversationId) {
      debugPrint(
          '📱 ConversationScreen: didUpdateWidget conversationId changed from ${oldWidget.conversationId} to ${widget.conversationId}');
      ...
      WidgetsBinding.instance.addPostFrameCallback((_) {
        ...
        if (widget.conversationId != null) {
          debugPrint(
              '📱 ConversationScreen: Switching to conversation ${widget.conversationId}');
          ref
              .read(chatProvider.notifier)
              .selectConversation(widget.conversationId!);
        } else {
  ...
  Widget build(BuildContext context) {
    debugPrint(
        '🛠️ ConversationScreen: build called. ID: ${widget.conversationId}, Active: ${ref.watch(activeConversationProvider)?.id}');
      ...
      if (next != null) {
        ...
        // Case 2: New messages added to current conversation
          ...
          final hasNewUserMessage =
              nextMessages.sublist(prevMessages.length).any((m) => m.isUser);
    ...
    final isMismatch =
        !isNewChat && activeConversation?.id != widget.conversationId;
    final isFetchingHistory = !isNewChat &&
        chatState.isLoading &&
        (activeConversation == null || activeConversation.messages.isEmpty);
    final isLoading = (activeConversation == null && !isNewChat) ||
        isMismatch ||
        isFetchingHistory;
...
class _NewChatWelcomeState extends State<_NewChatWelcome> {
  ...
  static const _suggestions = [
    _SuggestionItem(
      ...
      question:
          'Show me the top selling products for the current financial year',
    ),
    _SuggestionItem(
      ...
      question:
          'Give me a total revenue summary for the current financial year',
    ),
    _SuggestionItem(
      ...
      question:
          'Who are the top 20 customers by business in the current financial year?',
    ),
    _SuggestionItem(
            ...
            children: [
              ...
              Text(
                ...
                style: Theme.of(context).textTheme.displayLarge?.copyWith(
                      ...
                      letterSpacing: -1.0,
                      color:
                          AppColors.textPrimary.withOpacity(0.2), // Faded look
                    ),
              ),
